setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/latenz-tc-10mbps/")
...
# Merge Data Frames #
#####################
latenzTc10mbps <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzTc10mbps <- rbind(latenzTc10mbps, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzTc10mbps$s_newid)

latenzTc10mbpsSep <- latenzTc10mbps %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzTc10mbps <- latenzTc10mbpsSep[,z]

setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/R_Analysis/")
save(latenzTc10mbps, file = "latenzTc10mbps.Rda")
