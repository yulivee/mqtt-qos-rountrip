setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/logs.pi")
...
for (i in 1:length(names)){
  ...
  times<-as.data.frame(matrix(nrow=6000, ncol=6)) # Create times Matces to store RTT in next step
...
# Merge Data Frames #
#####################
logsPi <- merge(get(namesTime[1]), get(namesTime[2]), by = "r_newid")

for (i in 1:length(namesTime)){
logsPi <- rbind(logsPi, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(logsPi$s_newid)

logsPiSep <- logsPi %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
logsPi <- logsPiSep[,z]
...
#par(mfrow = c(1, 1))
for (i in 1:length(namesTime)){
namesTime<-get(namesTime[i])
plot(logsPi$id, logsPi$rtt, title(namesTime[i]))
}
