setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/latenz-tc-10kbps/")
...
# Merge Data Frames #
#####################
latenzTc10kbps <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzTc10kbps <- rbind(latenzTc10kbps, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzTc10kbps$s_newid)

latenzTc10kbpsSep <- latenzTc10kbps %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzTc10kbps <- latenzTc10kbpsSep[,z]
...
save(latenzTc10kbps, file = "latenzTc10kbps.Rda")
