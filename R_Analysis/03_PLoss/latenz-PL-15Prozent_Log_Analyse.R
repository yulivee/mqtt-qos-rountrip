setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/15%-ploss/")
...
# Merge Data Frames #
#####################
latenzPL15proz <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzPL15proz <- rbind(latenzPL15proz, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzPL15proz$s_newid)

latenzPL15prozSep <- latenzPL15proz %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzPL15proz <- latenzPL15prozSep[,z]
...
save(latenzPL15proz, file = "latenzPL15proz.Rda")
...
par(mfrow = c(1, 1))
plot(rttQoS0$id, rttQoS0$rtt, main = "RTT Paketloss 15% (10KByte, 1PproSek)", 
