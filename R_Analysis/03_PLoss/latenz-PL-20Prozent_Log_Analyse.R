setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/20%-ploss/")
...
# Merge Data Frames #
#####################
latenzPL20proz <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzPL20proz <- rbind(latenzPL20proz, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzPL20proz$s_newid)

latenzPL20prozSep <- latenzPL20proz %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzPL20proz <- latenzPL20prozSep[,z]
...
save(latenzPL20proz, file = "latenzPL20proz.Rda")
...
plot(time$id, time$rtt, main = paste(namesTime[i]), type = "b")
}
...
par(mfrow = c(1, 1))
plot(rttQoS0$id, rttQoS0$rtt, main = "RTT Paketloss 20% (10KByte, 1PproSek)", 
...
legend("topleft", c("QoS0", "QoS1", "QoS2"), text.width = 2.5,
       col = c("black", "red", "blue"),
       text.col = "black", cex = 0.7  ,lwd = c(2, 2, 2),
