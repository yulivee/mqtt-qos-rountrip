setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/10%-ploss/")
...
# Merge Data Frames #
#####################
latenzPL10proz <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzPL10proz <- rbind(latenzPL10proz, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzPL10proz$s_newid)

latenzPL10prozSep <- latenzPL10proz %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzPL10proz <- latenzPL10prozSep[,z]
...
save(latenzPL10proz, file = "latenzPL10proz.Rda")
...
par(mfrow = c(1, 1))
plot(rttQoS0$id, rttQoS0$rtt, main = "RTT Paketloss 10% (10KByte, 1PproSek)", 
...
legend("topleft", c("QoS0", "QoS1", "QoS2"), text.width = 2.5,
       ...
       text.col = "black", cex = 0.65  ,lwd = c(2, 2, 2),
