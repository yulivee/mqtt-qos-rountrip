setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/30%-ploss/")
...
# Merge Data Frames #
#####################
latenzPL30proz <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzPL30proz <- rbind(latenzPL30proz, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzPL30proz$s_newid)

latenzPL30prozSep <- latenzPL30proz %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzPL30proz <- latenzPL30prozSep[,z]
...
save(latenzPL30proz, file = "latenzPL30proz.Rda")
...
par(mfrow = c(1, 1))
plot(rttQoS0$id, rttQoS0$rtt, main = "RTT Paketloss 5% (10KByte, 1PproSek)", 
...
legend("bottom", c("QoS0", "QoS1", "QoS2"), text.width = 2.5,
