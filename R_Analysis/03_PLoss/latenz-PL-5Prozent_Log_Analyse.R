setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/5%-ploss/")
...
# Merge Data Frames #
#####################
latenzPL5proz <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzPL5proz <- rbind(latenzPL5proz, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzPL5proz$s_newid)

latenzPL5prozSep <- latenzPL5proz %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzPL5proz <- latenzPL5prozSep[,z]
...
save(latenzPL5proz, file = "latenzPL5proz.Rda")
