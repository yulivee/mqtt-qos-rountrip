setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/25%-ploss/")
...
# Merge Data Frames #
#####################
latenzPL25proz <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzPL25proz <- rbind(latenzPL25proz, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzPL25proz$s_newid)

latenzPL25prozSep <- latenzPL25proz %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzPL25proz <- latenzPL25prozSep[,z]
...
save(latenzPL25proz, file = "latenzPL25proz.Rda")
...
par(mfrow = c(1, 1))
plot(rttQoS0$id, rttQoS0$rtt, main = "RTT Paketloss 25% (10KByte, 1PproSek)", 
...
legend("bottomright", c("QoS0", "QoS1", "QoS2"), text.width = 4,
