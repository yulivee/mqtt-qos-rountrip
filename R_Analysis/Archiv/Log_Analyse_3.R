...
# 1. Subsetting
set.seed(123)
train1 <- sample(seq_len(nrow(df)), size = 5000000)
...
# 2. Subsetting
set.seed(234)
train2 <- sample(seq_len(nrow(df)), size = 1000000)
...
# 3. Subsetting
set.seed(345)
train3 <- sample(seq_len(nrow(df)), size = 3000000)
...
# Colnames after reading data
colnames(df)<- c("Date", "Time", "Action", "Topic", "QoS", "Size", "ID")
...
sentTimes <- subset(train_2, Action=="sent")
recTimes <- subset(train_2, Action=="received")
#################
