setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/latenz-tc-100kbps/")
...
# Merge Data Frames #
#####################
latenzTc100kbps <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzTc100kbps <- rbind(latenzTc100kbps, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzTc100kbps$s_newid)

latenzTc100kbpsSep <- latenzTc100kbps %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzTc100kbps <- latenzTc100kbpsSep[,z]
latenzTc100kbps[,3] <- "100KByte"  ### Note: vmtl. 100kbps Messung, aber keine Umbenennung der Files! - Anpassung bei Splits
...
save(latenzTc100kbps, file = "latenzTc100kbps.Rda")
