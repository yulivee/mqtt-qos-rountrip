setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/latenz-volle-bandbreite/")
...
# Merge Data Frames #
#####################
latenzVB <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
latenzVB <- rbind(latenzVB, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenzVB$s_newid)

latenzVBSep <- latenzVB %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenzVB <- latenzVBSep[,z]
...
save(latenzVB, file = "latenzVB.Rda")
...
plot(latenzVB$id, latenzVB$rtt, title(namesTime[1]))
}
