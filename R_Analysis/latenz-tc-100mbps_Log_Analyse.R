setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/Git/mqtt-qos-rountrip/logs/latenz-tc-100mbps/")
...
for (i in 1:length(names)){
  ...
  times<-as.data.frame(matrix(nrow=20000, ncol=6)) # Create times Matces to store RTT in next step
...
#################
# Calculate RTT # -> Bug, dass SentTimes und RecTimes nicht alle befüllt werden! Nur wenn händisch i angegeben wird...
#################

# Fill times Data Frames with Sent TS and IDs
for(i in 1 : length(namesSent)){
  sentTimes<- get(paste(namesSent[i])) 
  times<- get(paste(namesTime[i])) # get funktion funktioniert - times NA ist normal an dieser Stelle weil noch nicht befüllt!
...
# Merge Data Frames #
#####################
latenztc100mbps <- merge(get(namesTime[1]), get(namesTime[2]))

for (i in 1:length(namesTime)){
  latenztc100mbps <- rbind(latenztc100mbps, get(namesTime[i]))
}

####################
# Split Topic Name #
####################
separate(latenztc100mbps$s_newid)

latenztc100mbpsSep <- latenztc100mbps %>% separate(s_newid, c("n1", "n2", "QoS", "Size", "Min", "n3", "Speed", "n4" ))
z <- c(-2, -3, -7, -9)
latenztc100mbps <- latenztc100mbpsSep[,z]

#############
# Save file #
#############

setwd("/home/lisa/Darmstadt/05_Speicher und Datennetze IoT/Praktikum/R_Analysis/")
save(latenztc100mbps, file = "latenztc100mbps.Rda")
...
for (i in 1:length(namesTime)){
  ...
  plot(latenztc100mbps$id, latenztc100mbps$rtt, title(namesTime[1]))
}
